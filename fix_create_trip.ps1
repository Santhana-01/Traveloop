...
Set-Content -Path 'src/pages/CreateTrip.js' -Value $content

$css = Get-Content 'src/styles/CreateTrip.css' -Raw
if ($css -notmatch 'checkbox-group') {
  Add-Content 'src/styles/CreateTrip.css' "`n/* Checkbox Group Styling */`n.checkbox-group {`n  display: flex;`n  align-items: center;`n  gap: 10px;`n  margin-bottom: 25px;`n}`n`n.checkbox-group input[type=\"checkbox\"] {`n  width: 20px;`n  height: 20px;`n  cursor: pointer;`n  accent-color: var(--aurora-color-1, #99B898);`n  margin: 0;`n}`n`n.checkbox-group label {`n  display: inline;`n  margin-bottom: 0;`n  font-weight: 500;`n  color: #2A363B;`n  font-size: 15px;`n  text-transform: none;`n  letter-spacing: normal;`n  cursor: pointer;`n  user-select: none;`n}`n"
}
Write-Host 'CreateTrip.js and CSS updated successfully'
